...
class KenBurnsAnimation : Equatable {
    ...
    init(targetImage: UIImageView, zoomIntensity: Double, durationRange: DurationRange, pansAcross: Bool) {
        ...
        duration = Double.random(min: durationRange.min, max: durationRange.max)
        startTime = CACurrentMediaTime()

        let zoomMin = 1 + (0.3 * zoomIntensity)
        let zoomMax = 1 + (1.4 * zoomIntensity)
        //zoom = Random.double(zoomMin, zoomMax)
        zoom = Double.random(min: zoomMin, max: zoomMax)
        ...
        if pansAcross {
            offsets = (
                ...
                y: Double.random(min: 0.3 * range.min, max: 0.7 * range.min)
            )
        } else {
            offsets = (
                //x: Random.double(range.min, range.max),
                x: Double.random(min: range.min, max: range.max),
                //y: Random.double(range.min, range.max)
                y: Double.random(min: range.min, max: range.max)
            )
        }
    }
    ...
    func update(_ width: CGFloat, _ height: CGFloat) {
        ...
        let position:CGPoint = currentPosition(width, height)
        
        targetImage.frame.origin.x = position.x
        targetImage.frame.origin.y = position.y
...
@objc public class KenBurnsImageView: UIView {
    ...
    func startNewAnimation() {
        ...
        currentImageView.frame.size = self.size
